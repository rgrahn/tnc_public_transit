...
#choose which surge threshold to use
all <- surge_16
...
time.df <- with(check, check[hour(DATETIME) >= 19 & hour(DATETIME) < 24, ])
#-----------------------------------------------------------------------------
...
df <- data.frame(RIDERSHIP = test$RIDERSHIP, current = test$current, BUS_COUNT = test$BUS_COUNT, ave_stops = test$ave_stops, temperature = test$temperature, RAIN = test$RAIN,
                 SNOW = test$SNOW, IN_north_376 = test$IN_north_376, OUT_south_376 = test$OUT_south_376, IN_south_279 = test$IN_south_279,
                 OUT_north_279 = test$OUT_north_279, IN_west_376 = test$IN_west_376, OUT_east_376 = test$OUT_east_376,
                 ES_Eastbound = test$ES_Eastbound, ES_Westbound = test$ES_Westbound, day_of_week = test$day_of_week,
                 month = test$month, TIME = test$TIME)

test1 <- na.omit(df)

# --------------------- Propensity score weighted regression ----------------------------
#             Choose the covariates to generate propensity scores
#----------------------------------------------------------------------------------------
w <- weightit(current ~  BUS_COUNT + ave_stops + temperature + 
                RAIN + SNOW +
                ...
                factor(day_of_week) + factor(month) + factor(TIME),
                data = test1, method = "ps", estimand = "ATT", m.threshold = 0.1)

summary(w)
w.trim <- w

#check balance of weighted covariates
bal.tab(w.trim, m.threshold = .1, disp.v.ratio = TRUE)

#run a propensity score weighted regression model
d.w <- svydesign(ids = ~1, weights = get.w(w.trim),
                 data = test1)

fit <- svyglm(RIDERSHIP ~  current + BUS_COUNT + ave_stops + temperature + 
                RAIN + SNOW +
                IN_north_376 + OUT_south_376 +
                IN_south_279 + OUT_north_279 +
                IN_west_376 + OUT_east_376 +
                #ES_Eastbound + ES_Westbound +
                factor(day_of_week) + factor(month) + factor(TIME), design = d.w)
summary(fit)
...
labels <- c("Intercept","Surge Indicator","Bus Count","Ave. Stop Count","Temperature",
              "Rain","Snow",
              "North I-376 (Inbound)","South I-376 (Outbound)","South I-279 (Inbound)",
...
table <- stargazer(model1,model2,model3, type = "latex", omit = "TIME", single.row = TRUE,
                     intercept.bottom = FALSE, omit.stat = c("ser", "f"), dep.var.caption = c(paste0(current_poi," (4pm-7pm)")),
